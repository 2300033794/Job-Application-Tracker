...
import { api, readLocalJson, writeLocalJson } from "../lib/api";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const storedJobs = readLocalJson("jobs", []);
    const storedData = readLocalJson("applicationData", []);
    ...
    const hydrateDashboard = async () => {
      try {
        const [serverJobs, serverUser] = await Promise.all([
          api.getJobs(),
          api.getUser(),
        ]);

        if (serverJobs.length > 0 || storedJobs.length === 0) {
          setJobs(serverJobs);
          writeLocalJson("jobs", serverJobs);
        }

        if (serverUser) {
          setUserName(serverUser.name || "");
          setUserEmail(serverUser.email || "");
          writeLocalJson("user", { ...serverUser, password: storedUser.password || "" });
        }
      } catch {
        setJobs(storedJobs);
        setApplicationData(storedData);
